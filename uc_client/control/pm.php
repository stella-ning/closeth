...
class pmcontrol extends base {
    ...
    function ondelete() {
        ...
        if(is_array($pmids)) {
            ...
            if($this->apps[$this->app['appid']]['type'] == 'UCHOME') {
                ...
                $id = $_ENV['pm']->deletepmbyplids($this->user['uid'], $this->input('pmids')); // modified by connor weng, replace deletepmbypmids with deletepmbyplids, maybe it is a bug of ecmall, not sure
            }
        } else {
    ...
    function onviewnode() {
        ...
        $pmid = $this->input('pmid');
        $type = 0;
        $pms = $_ENV['pm']->getpmbyplid($this->user['uid'], $pmid); // modified by Connor Weng replace getpmbypmid with getpmbyplid. Bug of ecmall?
