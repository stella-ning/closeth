...
//            ),
       ...
       $conditions_tt  = array();
        $_GET['goods_name']&&$conditions_tt[] = $_GET['goods_name'];
       $_GET['store_name']&& $conditions_tt[] = $_GET['store_name'];
        // 分类
...
//        ));
        $goods_list = $this->_goods_mod->get_list2(array(
            ...
            'order' => "add_time desc",
            'limit' => $page['limit'],
            'conditions_tt' => $conditions_tt
        ), null, false,  true,$total_found,$backkey);
//        echo '<br> total_found is '.$total_found;
        ...
        foreach ($goods_list as $key => $goods)
        {
            ...
            exist_brandarea($goods['store_id']) && $goods_list[$key]['brandarea_show'] = 1;
        }
		//print_r($goods_list);
        ...
        $total_found && $page['item_count'] = $total_found;
    ...
    function edit()
    {
        ...
        else
        {
			...
			if(!empty($_POST['sort_order']) && intval($_POST['sort_order']) > 0)
			{
   ...
   function ajax_col()
   {
       ...
       if (in_array($column ,array('goods_name', 'brand', 'closed','sort_order','realpic')))
       {
           $data[$column] = $value;
