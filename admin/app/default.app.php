...
class DefaultApp extends BackendApp
{
    ...
    function _get_news_in_a_week()
    {
        ...
        return array(
            ...
            'new_goods_qty' => $this->_get_goods_account(7),
            'new_order_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "order WHERE finished_time > '$a_week_ago' AND status = '" . ORDER_FINISHED . "'"),
        );
    }
   function _get_goods_account($qt){
       $conditions .= " AND g.add_time <= " . time() ;
       $qt&& $conditions.=" AND g.add_time >= " . (time() -$qt * 24 * 60 * 60);
        $goods_mod = & m('goods');
    ...
    function _get_stats()
    {
        ...
        return array(
            ...
            'goods_qty' =>$this->_get_goods_account(),
