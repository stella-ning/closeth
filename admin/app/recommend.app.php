...
class RecommendApp extends BackendApp
{
    ...
    function view_goods()
    {
        ...
        $goods_list = $goods_mod->find(array(
            ...
            'fields' => 'g.goods_name, s.store_id, s.store_name, g.cate_name, g.brand, recommended_goods.sort_order, recommended_goods.recom_img,g.closed, g.if_show, views',
    ...
    function do_upload(){
        ...
        if ($_FILES['recommend_img'] ['error'] == UPLOAD_ERR_OK) {
            $uploader = new Uploader();
            $uploader->allowed_type(IMAGE_FILE_TYPE);
            $uploader->addFile($_FILES['recommend_img']);
            $uploader->root_dir(ROOT_PATH);
            $image_url = $uploader->save('data/files/mall/template',  $uploader->random_filename());
        }
        ...
        if ($image_url) {
            ...
            $recom_mod->updateRelation('recommend_goods' ,$recom_id ,array($goods_id) , array('recom_img' => $image_url) );
            
            $this->show_message('upload_ok',
                'back_list', 'index.php?app=recommend&amp;act=view_goods&amp;id=' . $recom_id);
