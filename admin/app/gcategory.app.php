...
class GcategoryApp extends BackendApp
{
    ...
    function edit()
    {
        ...
        else
        {
            ...
            if (!$this->_gcategory_mod->unique(trim($data['cate_mname']), $data['parent_id'], $id))
            {
            	$this->show_warning('name_exist');
            	return;
            }
    ...
    function ajax_col()
    {
       ...
       $data   = array();
       if (in_array($column ,array('cate_name', 'if_show', 'sort_order','cate_mname')))
       {
           $data[$column] = $value;
           if($column == 'cate_name' || $column == 'cate_mname')
           {
               $gcategory = $this->_gcategory_mod->get_info($id);
    ...
    function export_weight()
    {
        ...
        $gcategories = $this->_gcategory_mod->get_list();
        $tree =& $this->_tree($gcategories);
        $csv_data = $tree->getCSVData(0, array('cate_id', 'weight'));
        $this->export_to_csv($csv_data, 'gcategory', $to_charset);
    }
    ...
    //         $fileds = array_intersect($data[0],array('cate_name', 'weight')); //第一行含有的字段

    //         foreach ($data as $row)
    //         {
    //             if(array_intersect($row,array('cate_name', 'weight')))
    //             {
    //                 continue;
    //             }

    //             $cate_name_col = array_search('cate_name', $fileds); //从表头得到cate_name的列号
    //             $cate_name = isset($row[$cate_name_col]) ? trim($row[$cate_name_col]) : '';

    //             $weight_col = array_search('weight', $fileds); //从表头得到weight的列号
    //             $weight = is_numeric($weight_col) && isset($row[$weight_col]) ? $row[$weight_col] : 0;

    //             $cate_name = iconv('GBK', 'UTF-8', $cate_name);
    //             $this->_my_gcategory_mod =& m('gcategory');
    //             $gcategory = $this->_my_gcategory_mod->findAll(array(
    
    //                     'conditions'    => "cate_name = '$cate_name'",
    //                 ));

    //             if ($gcategory){
    //                 foreach ($gcategory as $key => $value) {

    //                     $conditions = 'cate_id = '.$value['cate_id'];

    //                     $this->_gcategory_mod->edit($conditions,array(
    //                         'weight'        => $weight
    //                     ));
    ...
    function import_weight()
    {
        ...
        else
        {
            ...
            $fileds = array_intersect($data[0],array('cate_name', 'weight', 'cate_id')); //第一行含有的字段

            foreach ($data as $row)
            {
                if(array_intersect($row,array('cate_name', 'weight', 'cate_id')))
                {
                    continue;
                }
                $cate_id_col = array_search('cate_id', $fileds); //从表头得到cate_id的列号
                $cate_id = isset($row[$cate_id_col]) ? $row[$cate_id_col] : 0;

                $weight_col = array_search('weight', $fileds); //从表头得到weight的列号
                $weight = is_numeric($weight_col) && isset($row[$weight_col]) ? $row[$weight_col] : 0;

               
                $gcategory = $this->_gcategory_mod->get("cate_id = $cate_id");

                if ($gcategory)
                {  
                    $condition = "cate_id = ".$gcategory['cate_id'];
                    $this->_gcategory_mod->edit($condition,array(
                        'weight'       => $weight,
                    ));
                }
