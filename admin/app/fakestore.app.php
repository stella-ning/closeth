...
class FakeStoreApp extends FakeBackendApp {

    var $_store_mod;
    var $floors  = array( '1' => '\'1F\',\'一楼\'',
                          '2' => '\'2F\',\'二楼\'',
                          '3' => '\'3F\',\'三楼\'',
                          '4' => '\'4F\',\'四楼\'',
                          '5' => '\'5F\',\'五楼\'',
    ...
    /**
     *  调用方式： http://serverAddressRoot/admin/index.php?app=fakestore&act=updateStores 
     * 约定： （1）旧版本所有的ecm_store中 floor为1,2,3,4,5,6；对应新版本在ecm_market里的mk_name为1F或者一楼
     *        (2) 调用此接口会遍历所有的店铺，让其为mk_id填充正确的值。
     */
    function updateStores() {
        $all = $this->_store_mod->getAll('select * from ecm_store');
        header("Content-type:text/html;charset=utf-8");
        $i = 0;
        foreach ($all as $one) {
            $store_id = $one['store_id'];
           // echo '<br>' . 'this is store '.$store_id.' serial is : ' . $i++ . '...........';
            if (!empty($one['shop_mall'])) {
                $shop_mall = $one['shop_mall'];
                //查出id from market 
                $sql = 'select mk_id,mk_name from ecm_market where mk_name=\'' . $shop_mall . '\'';
                $market = $this->_store_mod->getAll($sql);
                if (empty($market) || sizeof($market) == 0) {
                    echo 'no market';
                    continue;
                }
                $mk_name = $market[0]['mk_name'];
                $market_id = $market[0]['mk_id'];
                echo 'market1 id is :' . $market_id;
                if (!empty($one['floor'])) {
                    $floor = $one['floor'];
                    //根据id 查出最后的market id 
                    $sql = 'select * from ecm_market where mk_name in ('.$this->floors[$floor]. ') and parent_id=' . $market_id;
                    echo $sql;
                    $market = $this->_store_mod->getAll($sql);
                    if (empty($market) || sizeof($market) == 0 ) {
                        echo 'no floor';
                        ...
                        $market_id = $market[0]['mk_id'];
                        $mk_name.= '-'.$market[0]['mk_name'];
                        echo 'market2 id is :' . $market_id;
                    }
                }
                $this->_store_mod->edit('store_id='.$store_id,array('mk_id'=> $market_id,'mk_name' => $mk_name));
                echo 'store_id = '.$store_id.'---mk_id='.$market_id.'--mk_name='.$mk_name.'--updated!<br>';
            }
        }
    }
...
//                'tel'=> $_POST['tel'],      
                ...
                'has_link' => $_POST['has_link'],
                'serv_refund' => $_POST['serv_refund'],
                'serv_exchgoods' => $_POST['serv_exchgoods'],
                'serv_sendgoods' => $_POST['serv_sendgoods'],
                'serv_probexch' => $_POST['serv_probexch'],
                'serv_deltpic' => $_POST['serv_deltpic'],
                'serv_modpic' => $_POST['serv_modpic'],
                'serv_golden' => $_POST['serv_golden'],
            );
            $certs = array();
            ...
            //-------------begin of add shipping
            $data = array(
                'store_id' =>$user_id,
                'shipping_name' => '网站默认快递',
                'shipping_desc' =>'网站默认快递',
                'first_price' => 10,
                'step_price' => 0,
                'enabled' => 1,
                'sort_order' => 255,
            ...
            //-------------end of add shipping
            $this->show_message('add_ok', 'back_list', 'index.php?app=store', 'continue_add', 'index.php?app=store&amp;act=test'
            );
        }
    }
    ...
    function batch_edit() {
        if (!IS_POST) {
            ...
            $ret_page = isset($_GET['ret_page']) ? intval($_GET['ret_page']) : 1;
            $this->show_message('edit_ok', 'back_list', 'index.php?app=store&page=' . $ret_page);
        }
    }
