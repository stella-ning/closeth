...
class CashierApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        $orderextm_info = $orderextm_model->get("order_id={$order_id}");
		//配送方式
		$delivery =& m('delivery');
		$dl_name = $delivery->get("dl_id={$orderextm_info['dl_id']}");
		$orderextm_info['dl_name'] = $dl_name['dl_name'];
        /*查找是否代发订单*/
        /* if(!empty($order_info))
        {
            $orderbehalfs_model = & m('orderbehalfs');
            $behalf_order_info = $orderbehalfs_model->find('order_id = '.$order_id);
            if(count($behalf_order_info) == 1)
                foreach ($behalf_order_info as $boi)
                    $order_info['bh_id'] = $boi['bh_id'];
        }  */
        ...
        { 
            ...
            {
                $payments = $payment_model->get_enabled($order_info['bh_id']);
                if (empty($payments))
                { 
                    $this->show_warning('behalf_no_payment');
                    return;
                }
            }
            ...
            $this->assign("orderextm",$orderextm_info);
        ...
        { 
            ...
            {
                $payment_info  = $payment_model->get("payment_code = '{$order_info['payment_code']}' AND store_id={$order_info['bh_id']}");
            }
