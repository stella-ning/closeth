...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        /* 判断是否是淘宝采集店铺, 如是则不能编辑 */
         ...
         if ($store['shop_http']) {
            $this->show_warning(LANG::get('cannot_edit_taobao'));
            return;
         } 
        ...
        else
        {
             ...
             if ($_POST['recommended'] >= 0)
             {
                 $data['recommended'] = $_POST['recommended'] ? 1 : 0;
                         ...
                         if($_POST['delivery_template_id'] > 0)
                         {
                                 $data['delivery_template_id'] = intval($_POST['delivery_template_id']);
                         }
                         if($this->_filter_price($_POST['delivery_weight']) > 0)
                         {
                                $data['delivery_weight'] = $this->_filter_price($_POST['delivery_weight']);
                         }
             if ($data)
             {
   ...
   function ajax_col()
   {
       ...
       elseif(in_array($column, array('brandarea')))
       {
           $mod_store_brandarea_goods = & m('storebrandareagoods');
           $store_brandarea_goods = $mod_store_brandarea_goods->get($id);
           if($value && !$store_brandarea_goods)
           {
               $mod_store_brandarea_goods->add(array('goods_id'=>$id));
           }
           elseif(!$value && $store_brandarea_goods)
           {
               $mod_store_brandarea_goods->drop($id);
           }
           $store_brandarea_goods = $mod_store_brandarea_goods->get($id);
           if($store_brandarea_goods)
           {
               $this->json_result(1);
               return;
           }
    ...
    function _addible()
    {
        ...
        if (empty($payments))
        {
         ...
         if(empty($delivery))
         {
            $this->show_warning('delivery_template_empty', 'add_delivery_template', 'index.php?app=my_delivery');
            return false;
    ...
    function _get_post_data($id = 0)
    {
        $goods = array(
            ...
            'recommended'      => $_POST['recommended'],
                ...
                'delivery_weight' => $this->_filter_price($_POST['delivery_weight']),//tiq
    ...
    function _check_post_data($data, $id = 0)
    {
		...
		if(!isset($data['goods']['delivery_template_id']) || intval($data['goods']['delivery_template_id'])<=0){
			...
			return; */
			$data['goods']['delivery_template_id'] = 0;
		}
		//tiq
		else{
			$delivery_mod = &m('delivery_template');
			$cur_delivery = $delivery_mod->find(array(
					'conditions'=>'template_id='.intval($data['goods']['delivery_template_id']).' AND store_id='.$this->_store_id,						
			));
			if(!empty($cur_delivery)) $cur_delivery = array_values($cur_delivery);
			if($cur_delivery[0]['price_type'] == 2)
			{
				if(!isset($data['goods']['delivery_weight']) || floatval($data['goods']['delivery_weight'])<=0.00){
					$this->_error('no_delivery_weight_value');
					return;
				}
			}	
		}
    ...
    function _save_post_data($data, $id = 0)
    {
        ...
        $attribute_mod =& m('attribute');
        if ($id > 0) {
            /* 删除商品属性 */
            $goods_attr = $goods_attr_mod->find(array(
                'conditions' => "goods_id = '{$id}'",
                'fields' => 'gattr_id'
            ));
            $drop_gattr_ids = array_keys($goods_attr);
            if (!empty($drop_gattr_ids))
            {
                $goods_attr_mod->drop($drop_gattr_ids);
            }
        }
        $attrs = $_POST['attrs'].'1:1:商家编码:'.$this->makeOuterId($data);
        foreach (split(';', $attrs) as $attrsString) {
            if ($attrsString != '') {
                $attrsParts = split(':', $attrsString);
                if ($attrsParts[0] != '') {
                    $attribute_mod->add(array(
                        'attr_id' => $attrsParts[0],
                        'attr_name' => $attrsParts[2],
                        'input_mode' => 'select',
                        'def_value' => '其他',
                    ), true);
                    $goods_attr_mod->add(array(
                        'goods_id' => $goods_id,
                        'attr_name' => $attrsParts[2],
                        'attr_value' => $attrsParts[3],
                        'attr_id' => $attrsParts[0],
                        'value_id' => $attrsParts[1],
                    ));
                }
            }
        }
    ...
    function makeOuterId($data) {
        ...
        $huoHao = $this->getHuoHao($data['goods']['goods_name']);
        return $outerId = $seller.'_P'.$price.'_'.$huoHao.'#';
    }

    function getHuoHao($title) {
        $kuanHaoRegex='/[A-Z]?\d+/';
    ...
    function makeAttrsHtml() {
        $cid = $_REQUEST['cid'];
        $goodsId = $_REQUEST['goodsId'] == '' ? '' : $_REQUEST['goodsId'];
        $mod_gcategory = &m('gcategory');
        $cates = $mod_gcategory->get_list($cid, true);
        if (count($cates) === 0) {
            $html = file_get_contents('http://yjsc.51zwd.com/taobao-upload-multi-store/index.php?g=Taobao&m=Api&a=makePropsHtml&cid='.$cid.'&db=ecmall&goodsId='.$goodsId);
            header("Content-type:text/html;charset=utf-8");
            $html = preg_replace("#\\\u([0-9a-f]{4})#ie", "iconv('UCS-2BE', 'UTF-8', pack('H4', '\\1'))", $html);
            $html = stripslashes($html);
            echo(substr($html, 1, strlen($html) - 2));
        } else {
            echo('');
        }
    }
    
    /*ajax 检测新增商品名称是否存在 tiq*/
    function check_goodsname()
    { 
    	$goods_result = $this->_goods_mod->find(array(
    			'conditions' => " g.goods_name = '".trim($_GET['goods_name'])."'",
    	));
    	//初始化 result  false:有同名，true:无同名    	
    	$result = count($goods_result) > 0 ?false:true;
    	
    	///编辑商品
    	if($_GET['id'])
    	{
    	    if(count($goods_result) == 1 && trim($goods_result[$_GET['id']]['goods_name']) == trim($_GET['goods_name']))
    	    	$result = true;
    ...
    function recommend_brandarea()
    {
        ...
        {
            $goods_list[$key]['cate_name'] = $this->_goods_mod->format_cate_name($goods['cate_name']);
            $store_brandarea_goods = $mod_store_brandarea_goods->get($goods['goods_id']);
            $store_brandarea_goods && $goods_list[$key]['brandarea'] = 1; 
        }
        $this->assign('goods_list', $goods_list);
        ...
        $this->_curlocal(LANG::get('member_center'), 'index.php?app=member',
            LANG::get('my_goods'), 'index.php?app=my_goods',
            LANG::get('my_recommend_brandarea'));
        $this->_curitem('my_recommend_brandarea');
        $this->_curmenu('my_recommend_brandarea');
        //$this->import_resource(array('script' => 'utils.js'));
        $this->_config_seo('title', Lang::get('member_center') . ' - ' . Lang::get('my_goods'));
        $this->assign('goods_ids', implode(',', array_keys($all_goods)));
        $this->assign('store', $store);
        $this->display('my_goods.recommend_brandarea.html');
