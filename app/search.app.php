...
define('NUM_PER_PAGE', 100);        // 每页显示数量
...
//            echo ' come the initial process' . 'the count is ' . count($_GET);
         ...
         /* $sale_keywords=array(
             'T恤'=>array('6185592','6071970','5669817','5670139','5670166','5688257','5688253'),
             '毛衣'=>array('6071970'),
             '打底衫'=>array('6071970'),
             '针织衫'=>array('6071970'),
             '棉衣'=>array('6236837','5834484','6240221'),
             '羽绒服'=>array('6236837','5834484'),
             '修身'=>array('6236837'),
             '长款棉衣 '=>array('6236837','6240221'),
             '加厚显瘦棉衣'=>array('5834484'),
             '羽绒棉服'=>array('5834484'),
             '显瘦棉服'=>array('6240221'),
             '大摆棉衣'=>array('6240221')
         );   */
         
         $goods_list_keyword = array();
         if(!empty($param['keyword']) && !empty($sale_keywords))
         {
             foreach ($param['keyword'] as $pkw)
             {
                 if(in_array($pkw, array_keys($sale_keywords)))
                 {
                     $goods_list_keyword_result = $goods_mod->get_list(array(
                         'conditions' => "g.if_show = 1 AND g.closed = 0 AND g.default_spec > 0 AND s.state = 1 AND "
                         .db_create_in($sale_keywords[$pkw],'g.goods_id'),
                         'order' =>"field('g.goods_id',".join(',',$sale_keywords[$pkw]).")"
                     ));
                     $goods_list_keyword = array_merge($goods_list_keyword,$goods_list_keyword_result);
                 }
             }
         }
         if($param['cate_id'] == '50000671' )
         {
              
             $goods_list_keyword = $goods_mod->get_list(array(
                 'conditions' => "g.if_show = 1 AND g.closed = 0 AND g.default_spec > 0 AND s.state = 1 AND "
                 .db_create_in(array('6185592'),'g.goods_id') 
             ));
         }
         //搜索关键字广告      50000671为女装T恤
        /*  if($this->_is_include_keyword("T恤",$param['keyword']) || $param['cate_id'] == '50000671' )
         {          
             
             $goods_list_keyword = $goods_mod->get_list(array(
                 'conditions' => "g.if_show = 1 AND g.closed = 0 AND g.default_spec > 0 AND s.state = 1 AND "
                    .db_create_in(array('6185592','6071970','5669817','5670139','5670166','5688257','5688253'),'g.goods_id'),
                 'order' =>"field('g.goods_id','6185592','6071970','5669817','5670139','5670166','5688257','5688253')"
             ));
         }
         
         if($this->_is_include_keyword(array("毛衣","打底衫","针织衫"),$param['keyword'])  )
         {
             $goods_list_keyword = $goods_mod->get_list(array(
                 'conditions' => "g.if_show = 1 AND g.closed = 0 AND g.default_spec > 0 AND s.state = 1 AND ".db_create_in(array('6071970'),'g.goods_id'),
...
//            $tb001 = $this->getMillisecond();
            ...
            $goods_list = $goods_mod->get_list2(array(
                ...
                'order' => isset($_GET['order']) && isset($orders[trim(str_replace('asc', '', str_replace('desc', '', $_GET['order'])))]) ? $_GET['order'] : 'score desc', // tyioocom
            ...
            //合并关键字广告和搜索结果。
            if(!empty($goods_list_keyword) && !empty($goods_list))
            {
                foreach ($goods_list_keyword as $goods_keyword)
                {
                    array_unshift($goods_list, $goods_keyword);
                }
            }
...
//        echo '<br> after display ' . ($tb004 - $tb003);
    ...
    /**
     * 是否包含
     * @param 购买的关键字 $keyword
     * @param 用户搜索字符串 $inp
     */
    private function _is_include_keyword($keyword,$inp)
    {
        $result = false;
...
//        $t00 = time();
        ...
        // 查询参数
        $param = $this->_get_query_param(); //vdump($param);
       ...
       // $this->assign('filters', $this->_get_filter($param));
        ...
        $stats = $this->_get_group_by_info($param, ENABLE_SEARCH_CACHE);
        //显示当前状态--市场
        foreach ($stats['by_market'] as $key => $value) {
            if($_GET['mkid'] == $value['mk_id'])
            {
                $stats['by_market'][$key]['target'] = 1;
            }else{
                $stats['by_market'][$key]['target'] = 0;
            }
            
        }
        //显示当前状态--分类
        
        foreach ($stats['by_category'] as $key => $value) {
            if($_GET['cateid'] == $value['cate_id'])
            {
                $stats['by_category'][$key]['target'] = 1;
            }else{
                $stats['by_category'][$key]['target'] = 0;
            }
            
        }

        // print_r($stats['by_market']);exit;
        $this->assign('categories', $stats['by_category']);
        $this->assign('markets', $stats['by_market']);
        $this->assign('floors', $stats['by_market_floor']);
        $this->assign('price_intervals', $stats['by_price']);
        ...
        $this->assign("newStyleTime", $datearray);
        /* 排序 */
        $orders = $this->_get_orders();
        $this->assign('orders', $orders);
...
//            $conditions_tt = $this->_get_conditions_by_keyword($param['keyword'], ENABLE_SEARCH_CACHE);
//        }
        /* 每日新款 加入条件' AND g.add_time >='.mktime(0,0,0,date('m'),date('d'),date('Y')) 时间大于本日0点0分0秒 */
        $goods_list = $goods_mod->get_list2(array(
            'conditions' => $conditions,
        ...
        if (count($_GET) == 2) {
            $recom_mod = & m('recommend');
            $recommended_goods_list = $recom_mod->get_my_recommended_goods(SEARCH_RECOMMEND_NEWSGOODSTYPE);//by tanaiquan 2015-09-25
            /* $recom_goods_list = $recom_mod->get_recommended_goods(SEARCH_RECOMMEND_NEWSGOODSTYPE, NUM_PER_PAGE, true, 0);
            if (count($recom_goods_list) > 0) {
                $tmp_recom_goods_id = array();
                foreach ($recom_goods_list as $rec_goods) {
                    $tmp_recom_goods_id[] = $rec_goods['goods_id'];
                }
                $conditions = 'if_show=1 AND closed=0 AND ' . db_create_in($tmp_recom_goods_id, 'g.goods_id');

                $recommended_goods_list = $goods_mod->get_list(array(
                    'conditions' => $conditions,
                    'fields' => 's.praise_rate,s.im_qq,s.im_ww,', // tyioocom
                        ));
            } */
        ...
        if (!empty($recommended_goods_list)) {
            //添加zwd51_recommended用于表示为51网推荐
            foreach ($recommended_goods_list as $rkey => $rgoods) {
                $recommended_goods_list[$rkey]['zwd51_recommended'] = 1;
            }

            if (empty($goods_list)) {
                $goods_list = $recommended_goods_list;
            } else {
                foreach ($goods_list as $key => $goods) {
                    array_push($recommended_goods_list, $goods);
                    /* if (!in_array($key, $tmp_recom_goods_id)) {
                        array_push($recommended_goods_list, $goods);
                    } */
                }
                $goods_list = $recommended_goods_list;
        ...
        $order = $_GET['order'] ? urldecode($_GET['order']) : 'add_time desc';
        ...
        /* 配置seo信息 */
        $this->_config_seo($this->_get_seo_info('newgoods', $cate_id));
		$this->assign("pagecss",true);
        $t01 = time();
//       echo 'search new goods ;'.($t01-$t00). '<br>';
        $this->display('search.newgoods.html');
    }
    ...
    function _get_goods_conditions($param) {
        ...
        //筛选服务
        if (isset($param['service_detail'])) {
            $conditions .= " AND s.serv_realpic = " . $param['service_detail'];//g.realpic 更改为 g.serv_realpic
        }
        if (isset($param['service_send'])) {
...
//        }
    ...
    function get_store_info() {
        ...
        $rates = $mod_store->get_store_rates($sid);
