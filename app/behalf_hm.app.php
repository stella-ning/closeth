...
class Behalf_hmApp extends MallbaseApp {
    /* 搜索商品 */
...
//        $t00 = time();
        ...
        $behalf_hm_stores = $this->_get_behalf_hm_stores();
        if(empty($behalf_hm_stores))
        {
            $conditions_store_ids = " AND g.store_id = '0' ";
        }
        else 
        {
            $conditions_store_ids = " AND g.store_id ".db_create_in($behalf_hm_stores);
        }
...
//       echo 'index new goods ;'.($t01-$t00). '<br>';	
        ...
        $this->display('behalf.hm.goods.html');
//        $tb004 = $this->getMillisecond();
//        echo '<br> after display ' . ($tb004 - $tb003);
    }
    private function _get_behalf_hm_stores()
    {
        /* 筛选条件 */
        $model_storehm = & m('storehm');
        $results = $model_storehm->getCol("SELECT store_id FROM ".$model_storehm->table." WHERE state='1'");
    ...
    function store() {
        ...
        if ($isfirst || count($_GET) > 2) {
            ...
            $model_storehm = & m('storehm');
            $bhm_stores_ids = $model_storehm->getCol("SELECT store_id FROM ".$model_storehm->table);
            $conditions .=" AND ".db_create_in($bhm_stores_ids,'s.store_id');
    ...
    //print_r($storelist);
        ...
        $this->display('behalf.hm.store.html');
    }
