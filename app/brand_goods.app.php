...
class Brand_goodsApp extends MallbaseApp {
    /* 搜索商品 */
...
//        $t00 = time();
        ...
        $brand_area_stores = $this->_get_brand_area_stores();
        if(empty($brand_area_stores))
        {
            $conditions_store_ids = " AND g.store_id = '0' ";
        }
        else 
        {
            $conditions_store_ids = " AND g.store_id ".db_create_in($brand_area_stores);
        } 
        $brand_area_goods=$this->_get_brand_area_goods();
     
        if(empty($brand_area_goods))
        {
            $conditions_goods_ids = " AND g.store_id = '0' ";
        }
        else 
        {
            $conditions_goods_ids = " AND g.goods_id ".db_create_in($brand_area_goods);
        } 
       
        //dump($conditions_goods_ids);
...
//            echo ' come the initial process' . 'the count is ' . count($_GET);
            ...
            $conditions .= $conditions_goods_ids; 
...
//       echo 'index new goods ;'.($t01-$t00). '<br>';	
        ...
        //$this->assign('apply_btn',array('text'=>Lang::get('apply_join_behalf_area'),'url'=>'index.php?app=behalf_goods&act=apply'));
        $this->display('brand.goods.wind.html');
//        $tb004 = $this->getMillisecond();
//        echo '<br> after display ' . ($tb004 - $tb003);
    }
    private function _get_brand_area_stores()
    {
        /* 筛选条件 */
        $model_storebrandarea = & m('storebrandarea');
        $results = $model_storebrandarea->getCol("SELECT store_id FROM ".$model_storebrandarea->table." WHERE state='1'");

        return $results;
    }
    
    private function _get_brand_area_goods()
    {
        $mod_storebrandgoods = & m('storebrandareagoods');
        $results = $mod_storebrandgoods->getCol("SELECT goods_id FROM ".$mod_storebrandgoods->table." WHERE state='1'");
        return $results;
    }
    ...
    function apply()
    {
        ...
        $storebrandarea_model = & m('storebehalfarea');
        $storebrandarea = $storebrandarea_model->get($user_id);
        
        if ($storebrandarea['state'] == 1)
        {
            $this->show_warning('user_has_behalf_area');
            return;
        }
        elseif($storebrandarea['state'] === 0)
        {
        ...
        else 
        {
            ...
            $affect_rows = $storebrandarea_model->add(array('store_id'=>$user_id,'state'=>'1','category'=>'freeze'));
    ...
    function store() {
        ...
        if ($isfirst || count($_GET) > 2) {
            ...
            $model_storebrandarea = & m('storebrandarea');
            $ba_stores_ids = $model_storebrandarea->getCol("SELECT store_id FROM ".$model_storebrandarea->table." WHERE state='1'");
    ...
    //print_r($storelist);
        ...
        $this->display('brand.goods.store.html');
    }
