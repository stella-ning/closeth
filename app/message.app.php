...
class MessageApp extends MemberbaseApp
{
    ...
    function view()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->assign('box', $box);
            $this->assign('plid', $msg_id); // added by connor weng
            /* 当前位置 */
            $this->_curlocal(LANG::get('member_center'),   'index.php?app=member',
                             LANG::get('message'),         'index.php?app=message&amp;act=newpm',
        ...
        else
        {
            ...
            $msg_id = $ms->pm->send($this->visitor->get('user_id'), $reply_to_id, '', $_POST['msg_content'] , $msg_id); // 获取msg_id
            if ($msg_id < 0)
            {
                $this->show_warning($ms->pm->get_error());
    ...
    function drop()
    {
        $msg_ids = isset($_GET['msg_id']) ? intval(trim($_GET['msg_id'])) : '';
