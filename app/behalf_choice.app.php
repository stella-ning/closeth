...
class Behalf_choiceApp extends MallbaseApp {
    /* 搜索商品 */
...
//        $t00 = time();
        ...
        $behalf_area_stores = $this->_get_behalf_area_stores();
...
//            echo ' come the initial process' . 'the count is ' . count($_GET);
            ...
            //BEHALF_CHOICE_RECOMMENDED_TYPE
            $recommend_mod = & m('recommend');
            $recommend_goods_ids = $recommend_mod->getRelatedData("recommend_goods",BEHALF_CHOICE_RECOMMENDED_TYPE,array('fields'=>'g.goods_id'));
            if($recommend_goods_ids);
            {
                $rec_goods_ids = array();
                foreach ($recommend_goods_ids as $recommend_goods_id)
                {
                    $rec_goods_ids[] = $recommend_goods_id['goods_id'];
                }
                unset($recommend_goods_ids);
                
                $conditions .= " OR g.goods_id ".db_create_in($rec_goods_ids);
            }
...
//            $tb001 = $this->getMillisecond();
             ...
             $total_found = $goods_mod->getCount();
...
//       echo 'index new goods ;'.($t01-$t00). '<br>';	
        ...
        $this->assign("current_local_plaction",Lang::get('behalf_area'));
        $this->assign('apply_btn',array('text'=>Lang::get('apply_join_behalf_area'),'url'=>'index.php?app=behalf_choice&act=apply'));
        $this->display('behalf.choice.wind.html');
//        $tb004 = $this->getMillisecond();
//        echo '<br> after display ' . ($tb004 - $tb003);
    }
    private function _get_behalf_area_stores()
    {
        /* 筛选条件 */
        $model_storebehalfchoice = & m('storebehalfchoice');
        $results = $model_storebehalfchoice->getCol("SELECT store_id FROM ".$model_storebehalfchoice->table." WHERE state='1'");
    ...
    function apply()
    {
        ...
        $storebehalfarea_model = & m('storebehalfchoice');
        $storebehalfarea = $storebehalfarea_model->get($user_id);
        
        if ($storebehalfarea['state'] == 1)
        {
            $this->show_warning('user_has_behalf_area');
            return;
        }
        elseif($storebehalfarea['state'] === '0')
        {
        ...
        {            
            ...
            $this->display('behalf_choice.apply.step1.html');
        }
        else 
        {
           ...
           /*  include_once(ROOT_PATH.'/app/fakemoney.app.php');
            ...
            $affect_rows = $storebehalfarea_model->add(array('store_id'=>$user_id,'state'=>'1','category'=>'freeze'));
            ...
            $this->show_message('apply_ok','index', 'index.php?app=behalf_choice');
        }
    ...
    function store() {
        ...
        if ($isfirst || count($_GET) > 2) {
            ...
            //$model_storebehalfchoice = & m('storebehalfchoice');
            //$ba_stores_ids = $model_storebehalfchoice->getCol("SELECT store_id FROM ".$model_storebehalfchoice->table);
            $ba_stores_ids = $this->_get_behalf_area_stores();
            $conditions .=" AND ".db_create_in($ba_stores_ids,'s.store_id');
    ...
    //print_r($storelist);
        ...
        $this->display('behalf.choice.store.html');
    }
