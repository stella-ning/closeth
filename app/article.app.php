...
class ArticleApp extends MallbaseApp
{
    ...
    {   
        ...
        if(empty($_GET['keyword'])){
    		...
    		foreach($acategories as $k=>$v){
    			...
    			$sql = "SELECT article_id,title FROM ".DB_PREFIX."article WHERE if_show=1 AND cate_id=".$v['cate_id']." order by article_id limit 0,5";
    			$res[$k] = $db->getall($sql);
    			$acategories[$k]['article'] = $res[$k];
    		}
            ...
            $sql = "SELECT article_id,title FROM ".DB_PREFIX."article WHERE if_show=1 AND cate_id !='-1' AND title LIKE '%".$keyword."%'";
            $res = $db->getall($sql);
            $this->assign('keyw', true);
    ...
    function view()
    {
        ...
        /*文章分类*/
		$db = &db();
		$sql = 'SELECT cate_id,cate_name FROM '.DB_PREFIX.'acategory WHERE parent_id = 0 ORDER BY sort_order ASC LIMIT 0,5';
		$acategories = $db->getall($sql);
		foreach($acategories as $k=>$v){
	
			$sqla = 'SELECT article_id,title FROM '.DB_PREFIX.'article WHERE  cate_id='.$v['cate_id'].' AND if_show != 0 ORDER BY sort_order ASC';
			$resa = $db->getall($sqla);
			
			$acategories[$k]['article'] = $resa;
		}
       // $acategories = $this->_get_acategory($cate_id);
        /* 新文章 */
        //$new = $this->_get_article('new');
