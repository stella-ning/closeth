 ...
 *    @usage    none
 */
class My_behalf_paymentApp extends MemberbaseApp
{
    function index()
    {
        /* 取得列表数据 */
        ...
        $installed     = $model_payment->get_installed($this->visitor->get('has_behalf'));
        ...
        $this->display('my_behalf_payment.index.html');
    }
    ...
    function install()
    {
        ...
        $payment_info = $model_payment->get("store_id=" . $store_id . " AND payment_code='{$code}'");
        ...
        if (!IS_POST)
        {
            ...
            $this->_config_seo('title', Lang::get('member_center') . Lang::get('my_payment'));
            header("Content-Type:text/html;charset=" . CHARSET);
            $this->display('my_behalf_payment.form.html');
        }
        else
        {
            $data = array(
    ...
    function config()
    {
        ...
        $model_payment =& m('payment');
        $payment_info  = $model_payment->get("store_id = " . $this->visitor->get('has_behalf') . " AND payment_id={$payment_id}");
        if (!$payment_info)
        {
            echo Lang::get('no_such_payment');
        ...
        else
        {
            ...
            $model_payment->edit("store_id =" . $this->visitor->get('has_behalf') . " AND payment_id={$payment_id}", $data);
    ...
    function uninstall()
    {
        ...
        /*代发和店铺都可*/
        $store_id = null;
        if($this->visitor->get('has_behalf') || $this->visitor->get('has_behalf'))
        {
        	$store_id = $this->visitor->get('user_id');
        }

        $model_payment =& m('payment');
        $model_payment->uninstall($store_id, $payment_id);
        if ($model_payment->has_error())
        {
            $this->show_warning($model_payment->get_error());
