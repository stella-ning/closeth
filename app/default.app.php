...
//                    echo '系统数据刷新令牌成功！';
    ...
    function searchtrade() {
        ...
        if ($tokens && $tokens['result'] == 'success') {
            ...
            Log::write('error:oem is :'.$oem.'--aliacccount is :'.$aliaccount.'--tradeno is '.$tradeno.'--appid is '.APP_ID);
            exit(json_encode($finalresult));
        }
        ...
        if (empty($title) && !empty($tradeno)) {
            ...
            $start_time = date('Y-m-d H:i:s', time() - 6 * 24 * 60 * 60 + 1); //这个不耗性能
        }
        // 开始时间，时间必须是今天范围之内
...
//        print_r($datas);
                ...
                for ($j = 0; $j < intval($count / ALI_PAGE_SIZE) + 1; $j++) {//页码
                    ...
                    for ($i = 0; $i < (ALI_PAGE_SIZE < ($count - $j * ALI_PAGE_SIZE) ? ALI_PAGE_SIZE : ($count - $j * ALI_PAGE_SIZE)); $i++) {
                        ...
                        if ($tradeinfo ['in_out_type'] == 'in'
                            ...
                            $num++;
                            $spec[$tradeno] = array('money' => $money, 'title' => $title, 'tradeno' => $tradeno);
                        }
                    }
                }
            }
    ...
    function tradeSpec2() {
        ...
        $start_time = date('Y-m-d H:i:s', $nowtime - 7 * 24 * 60 * 60 + 1);
        $req1->setEndTime($end_time);
        if (!empty($tradeno)) {
            $req1->setAlipayOrderNo($tradeno);
        }
...
//        print_r($datas);
    ...
    public function checkMoney() {
        ...
        $checkresult['failedmoney'] = 0;
        $checkresult['successmoney'] = 0;
        $checkresult['ymoney'] = 0;
         $checkresult['equal']=substr(OEM,0,1).'_n';

        $url = SITE_URL . '/admin/index.php?app=fakemoney&act=todayforyes';
        $res = @json_decode($this->Get($url), true);
        $checkresult['ymoney'] = $res['ymoney'];

        $oem = OEM;
        if ($total < $alimoney + $checkresult['ymoney'] || $force) {
            ...
            foreach ($aliM['spec'] as $stat => $v) {
                ...
                if (!$t) {
                    ...
                    $spec['title'] = $memo; {
                        ...
                        if (!$r || !$user_idG || !$titleG || $user_idG==10 ||$user_idG==22687) {
                            $spec['result'] = 'failed';
                            $checkresult['failed']++;
                            $checkresult[$key] = $spec;
                            $checkresult['failedmoney']+=$money;
                            $checkresult['num']++;
                            if( $db){
                                $this->rememtrade($key, $memo, $money, 1);
                                $this->getyestoday();
                            }
                            continue;
                        }
...
//                        $url = "http://" . MONEYSITE . "/index.php?app=default&act=searchtrade&title=" . $title . "&totalfee=" . $total_fee . "&user_id=" . $user_id . "&user_name=" . $user_name . "&oem=" . $oem;
                        ...
                        if ($res['result'] == 'success') {
                            ...
                            $spec['result'] = 'success';
                            $checkresult['suc']++;
                            $checkresult['successmoney'] +=$money;
         ...
         if(bccomp($total + $checkresult['failedmoney'] + $checkresult['successmoney'] , $alimoney + $checkresult['ymoney'])==0 ){
              $checkresult['equal']= substr(OEM,0,1).'_E';
         }
//            print_r($checkresult);
        } else if ($total > $alimoney + $checkresult['ymoney']) {
           $url = SITE_URL . '/admin/index.php?app=fakemoney&act=todayextra&del=yes';
           $res = @json_decode($this->Get($url), true);
        } else {// ==
            $checkresult['equal']= substr(OEM,0,1).'_E';
        }
        $smail = get_mail('sms_money_notify', array('money' => $checkresult));
        if(strpos( $checkresult['equal'],'_E') === false){
          //$this->sendSaleSms(ADMINPHONE, addslashes($smail['message']));
        }
...
//                header("location: http://yjsk.51zwd.com/index.php?app=default&act=mymoney&tradeno=" . $trade_no . "&totalfee=" . $total_fee . "&user_id=" . $user_id . "&user_name=" . $user_name);
    ...
    function loginWithAjax() {
        if (!IS_POST) {
            ...
            if (!$user_id) {
                ...
                echo "<script type='text/javascript'>window.parent.layer.alert('" . Lang::get('login_failed') . "');</script>";
            ...
            echo "<script type='text/javascript'>window.parent.layer.alert('" . Lang::get('login_successed') . "');</script>";
    ...
    function refreshSphinx() {
        ...
        if (!$current) {
            ...
            keep($current,true);
            $cache_server->set('currentSphinx',$current, 0); //先用本地的，等更新好了再用主备
        }
        $assistant = $cache_server->get('assistantSphinx');
        if (!$assistant) {
            $assistant = (SECOND_SPHINX==$current?FIRST_SPHINX:SECOND_SPHINX);
            $cache_server->set('assistantSphinx', $assistant, 0); //备用的可以先设置好
        }
...
//              $this->cancleOrder($vals['order_id'], $vals['buyer_id'], $vals['order_amount']);
    ...
    function autoCancle123() {
        $order = &m('order');
        $my_moneylog_mod = & m('my_moneylog');
        $sql = 'select l.order_id ,l.caozuo ,l.leixing ,l.buyer_id, o.status,o.order_amount from ecm_my_moneylog l , ecm_order o  where  l.order_id=o.order_id and o.status=0 and l.caozuo!=30 and l.caozuo!=80 and l.s_and_z=2 ';
        $results = $order->db->getAll($sql);
        foreach ($results as $vals) {
            if(empty($vals['buyer_id'])) continue;
            echo 'order id ' . $vals['order_id'] . '-' . $vals['buyer_id'] . '-' . $vals['order_amount'] . '<br>';
...
//       $result = @json_decode(Get($url),true);
   ...
   function getyestoday(){
        $day = 8;
        $dd = 1;
        $moneylog_mod = & m('my_moneylog');
        $sql = 'SELECT id,money,order_sn, user_name, from_unixtime(add_time) ,moneyleft FROM `ecm_my_moneylog`
            where user_log_del=0 and leixing=30 and caozuo=4 and FROM_UNIXTIME(add_time) >date_sub(curdate(), interval ' . $day . ' hour)
                and FROM_UNIXTIME(add_time) <date_sub(curdate(), interval ' . ($day-24) . ' hour)
                and locate(  date_sub(curdate(), interval ' . ($dd-1) . ' day) +0 ,order_sn) = 0;';
        $result = $moneylog_mod->getAll($sql);
        foreach($result as $r){
            $this->rememtrade( $r['order_sn'], '',  $r['money'], 0);
        }
   }
    /**
     * $add = 1 ,增加; 0 修改
     */
    function rememtrade($alino, $title, $totalfee, $add) {
        $paylogmodel = &m('alitrade');
        if ($add == 1) {
            $condition ['ali_trade_no'] = $alino;
            ...
            $condition ['endtime'] = date("Y-m-d H:i:s", time());
            $condition ['trade_status'] = 1;
            @$paylogmodel->edit('ali_trade_no="' . $alino . '" and total_fee=' . $totalfee, $condition);
        }
    }
    function  testc(){
         require_once ROOT_PATH . '/app/fakemoney.app.php';
            $b = new FakeMoneyApp();
            $b->to_user_withdraw(2,10919,2,'test','33','33');
    }
     function  testb(){
         require_once ROOT_PATH . '/app/fakemoney.app.php';
            $b = new FakeMoneyApp();
           $r = $b->to_user_withdraw(10919,2,2,'test','33','33');
           if($r===true){
               echo 'suc';
           }else{
               echo 'fail'.$r;
    ...
    function yjsc() {
        $this->display('yjsc.html');
    }

    function close_stores_by_rules() {
        $nowtime = gmtime();
        $eight_weeks_ago = $nowtime - 60*60*24*7*8;
        $eight_weeks_ago_date = date('Ymd', $eight_weeks_ago);
        $two_weeks_ago = $nowtime - 60*60*24*7*2;
        $two_weeks_ago_date = date('Ymd', $two_weeks_ago);

        $store_mod = &m('store');
        $sql_rule1 = "select store_id,store_name,im_qq,mk_name,dangkou_address,tel from ecm_store where state = 1 and add_time < {$eight_weeks_ago} and store_id not in (select distinct store_id from analyze_yjsc where date > {$two_weeks_ago_date}) and store_id not in (select distinct seller_id from ecm_order where pay_time > {$eight_weeks_ago}) order by shop_mall, floor, dangkou_address";
        $stores_rule1 = $store_mod->getAll($sql_rule1);
        $sql_rule2 = "select s.store_id,s.store_name,s.im_qq,s.mk_name,s.dangkou_address,s.tel from ecm_store_rates r, ecm_store s where r.store_id = s.store_id and (r.lack_rate > 20 or r.praise_rate < 90 or r.back_rate > 10) order by s.shop_mall, s.floor, s.dangkou_address";
        $stores_rule2 = $store_mod->getAll($sql_rule2);
        if (!IS_POST) {
            $this->assign('eight_weeks_ago_date', $eight_weeks_ago_date);
            $this->assign('two_weeks_ago_date', $two_weeks_ago_date);
            $this->assign('stores_rule1', $stores_rule1);
            $this->assign('stores_rule2', $stores_rule2);
            $this->assign('total_rule1', count($stores_rule1));
            $this->assign('total_rule2', count($stores_rule2));
            $this->display('close_stores_by_rules.html');
        } else {
            foreach ($stores_rule1 as $store) {
                $rs1 = $store_mod->db->query("update ecm_store set state = 2, close_reason = 'rules' where store_id = {$store['store_id']}");
                if ($rs1) {
                    echo($store['store_id'].' updated\n');
                } else {
                    echo($store['store_id'].' update failed\n');
                }
                $rs2 = $store_mod->db->query("insert into ecm_store_log values ({$store['store_id']}, {$nowtime}, 0, 1)");
                if ($rs2) {
                    echo($store['store_id'].' logged\n');
                } else {
                    echo($store['store_id'].' log failed\n');
                }
            }
        }
    }
     ...
     * 展示广告，iframe
     */
    function show_goods_detail_advs()
    {
        $this->display("goods.adv_iframe.html");
    }
}
