...
//                        echo $tradeinfo ['order_title'] . "-" . $tradeinfo ['total_amount'] . "-" . $tradeinfo ['in_out_type'] . '<br>';
                        if (strpos($tradeinfo ['order_title'] ,$title)> -1 && $tradeinfo ['total_amount'] == $totalfee && $tradeinfo ['in_out_type'] == 'in'
...
//                        exit(json_encode($finalresult));
//                        $this->financeSpec2($paylogmodel, $user_id, $user_name, $totalfee, $title, $accesstoken, $oem, $time);
                        $this->financeSpec4($paylogmodel, $user_id, $user_name, $totalfee, $title, $accesstoken, $oem, $time);
                    }
                } else {
                    $finalresult['result'] = 'error';
        ...
        if (!empty($tradeno)) {
//            $req1->setAlipayOrderNo($tradeno);
            $this->financeSpec3($paylogmodel, $user_id, $user_name, $totalfee, $title, $accesstoken, $oem, $tradeno);
        }
        if ($datas ['response_error']) {
            echo ( '交易记录查询失败！' );
        } else if ($datas ['alipay_user_trade_search_response']) {
...
//                    echo '系统数据刷新令牌成功！';
        ...
        $start_time = date('Y-m-d H:i:s', $nowtime - 2 * 24 * 60 * 60 + 1);
        $req1->setEndTime($end_time);
        if (!empty($tradeno)) {
            $req1->setAlipayOrderNo($tradeno);
        }
...
//        print_r($datas);
        if ($datas ['response_error']) {
            ...
            if ($count > 0) {
                for ($i = 0; $i < $count; $i++) {
                    $tradeinfo = $datas ['alipay_user_account_search_response'] ['account_records'] ['account_record'] [$i];
                    if ($tradeinfo ['type'] == 'payment') {
                        $title = $tradeinfo ['alipay_order_no'];
                        $time = $tradeinfo ['create_time'];
                        $money = $tradeinfo ['in_amount'];
                        $memo = $tradeinfo ['memo']; //不一定有
                        $totalfee += $tradeinfo ['in_amount'];
                        $spec[$title] = array('money' => $money, 'create_time' => $time, 'memo' => $memo);
                    }
                }
            }
            if ($datas ['alipay_user_account_search_response'] ['total_pages'] > 1) {
                $req1->setPageNo("2");
                $resp1 = $c->execute($req1, $accesstoken);
                $datas = (array) $resp1;
//                echo '<br>--------------';
...
//        print_r($datas);
            ...
            if ($count > 0) {
                for ($i = 0; $i < $count; $i++) {
                    ...
                    if ($tradeinfo ['type'] == 'payment') {
                        ...
                        $money = $tradeinfo ['in_amount'];
                        $totalfee += $tradeinfo ['in_amount'];
                        $spec[$title] = array('money' => $money, 'create_time' => $time);
                    }
                }
            }
            $finalresult['result'] = 'success';
     ...
     * 查询最近3分钟的 
     */
    function financeSpec2($paylogmodel, $user_id, $user_name, $totalfee, $title, $accesstoken, $oem, $time = '') {
        // 交易记录查询接口
        $c = new AopClient ();
        $c->appId = APP_ID;
        $c->rsaPrivateKeyFilePath = ROOT_PATH . "/rsa_private_key.pem";
        ...
        // 结束时间。与开始时间间隔在七天之内
        $end_time = date('Y-m-d H:i:s', $nowtime);
        $start_time = date('Y-m-d H:i:s', $nowtime - 180);
        if ($time && !empty($time)) {
...
//        print_r($datas);
        ...
        if ($datas ['response_error']) {
            $finalresult['result'] = 'error';
            $finalresult['info'] = "account record failed!";
            exit(json_encode($finalresult));
        } else if ($datas ['alipay_user_account_search_response']) {
            $count = $datas ['alipay_user_account_search_response'] ['total_results'];
            $done = false;
            if ($count > 0) {
                for ($i = 0; $i < $count; $i++) {
                    $tradeinfo = $datas ['alipay_user_account_search_response'] ['account_records'] ['account_record'] [$i];
                    if ($tradeinfo ['type'] == 'payment' && $tradeinfo ['in_amount'] == $totalfee) {
                        $sql = 'SELECT out_trade_no from  ' . $paylogmodel->table . ' where out_trade_no="' . $tradeinfo ['alipay_order_no'] . '"';
                        $results = $paylogmodel->db->getOne($sql);
                        if ($results && !empty($results)) {
                            $finalresult['result'] = 'error';
                            $finalresult['info'] = "alipay_order_no-used-b!";
                            exit(json_encode($finalresult));
                        }
                        $sql = 'SELECT trade_no from  ' . $paylogmodel->table . ' where total_fee="' . $tradeinfo ['in_amount'] . '" and customer_id="' . $user_id . '" and trade_no="' . $title . '"';
                        $results = $paylogmodel->db->getOne($sql);
                        if ($results) {
                            $updateOuttrade = array(
                                'out_trade_no' => $tradeinfo ['alipay_order_no'],
                            ...
                            $paylogmodel->edit('trade_no="' . $title . '"', $updateOuttrade);
                        } else {
                            $condition ['out_trade_no'] = $tradeinfo ['alipay_order_no'];
                            $condition ['total_fee'] = $tradeinfo ['in_amount'];
                            $condition ['createtime'] = date("Y-m-d H:i:s", time());
                            $condition ['endtime'] = date("Y-m-d H:i:s", time());
                            $condition ['trade_status'] = 1;
                            $condition ['customer_id'] = $user_id;
...
//                            echo '站内充值成功';
                        ...
                        $my_money_mod->edit('user_id=' . $user_id, $edit_myjifen);
                        //添加日志
                        $log_text = '无手续费充值 in A2'; //$this->visitor->get('user_name') . Lang::get('tongguoalipaychongzhi') . $total_fee . Lang::get('yuan');
                ...
                if ($done) {
                    $finalresult['result'] = 'success';
                    $finalresult['info'] = "found-in-a2!";
                    exit(json_encode($finalresult));
                } else {
                    $finalresult['result'] = 'error';
                    $finalresult['info'] = "no-t; no-rs-f-in-a2";
                    exit(json_encode($finalresult));
                }
            } else {
                //没有记录
                $finalresult['result'] = 'error';
                $finalresult['info'] = "no-t; no-rs-in-a2";
                exit(json_encode($finalresult));
            }
            $finalresult['result'] = 'success';
            $finalresult['total'] = $totalfee;
     ...
     * @param type $time 
     */
    function financeSpec3($paylogmodel, $user_id, $user_name, $totalfee, $title, $accesstoken, $oem, $tradeno) {
        // 交易记录查询接口
        $c = new AopClient ();
        $c->appId = APP_ID;
        $c->rsaPrivateKeyFilePath = ROOT_PATH . "/rsa_private_key.pem";
...
//        print_r($datas);
        ...
        if ($datas ['response_error']) {
            ...
            if ($count > 0) {
                for ($i = 0; $i < $count; $i++) {
                    $tradeinfo = $datas ['alipay_user_account_search_response'] ['account_records'] ['account_record'] [$i];
                    if ($tradeinfo ['type'] == 'payment' && $tradeinfo ['in_amount'] == $totalfee && $tradeinfo['alipay_order_no'] == $tradeno) {
                        $sql = 'SELECT out_trade_no from  ' . $paylogmodel->table . ' where out_trade_no="' . $tradeinfo ['alipay_order_no'] . '"';
                        $results = $paylogmodel->db->getOne($sql);
                        if ($results && !empty($results)) {
                            $finalresult['result'] = 'error';
                        ...
                        $sql = 'SELECT trade_no from  ' . $paylogmodel->table . ' where total_fee="' . $tradeinfo ['in_amount'] . '" and customer_id="' . $user_id . '"  and trade_status=0';
                        if (!empty($title)) {
                            $sql = 'SELECT trade_no from  ' . $paylogmodel->table . ' where trade_no="' . $title . '" and total_fee="' . $tradeinfo ['in_amount'] . '" and customer_id="' . $user_id . '"  and trade_status=0';
                        }
                        $results = $paylogmodel->db->getRow($sql);
                        if ($results) {
                            $updateOuttrade = array(
                                'out_trade_no' => $tradeinfo ['alipay_order_no'],
                                'trade_status' => 1,
                            ...
                            $paylogmodel->edit('trade_no="' . $results['trade_no'] . '"', $updateOuttrade);
                        } else {
                            $condition ['out_trade_no'] = $tradeinfo ['alipay_order_no'];
                            $condition ['total_fee'] = $tradeinfo ['in_amount'];
                            $condition ['createtime'] = date("Y-m-d H:i:s", time());
...
//                            echo '站内充值成功';
                        ...
                        $my_money_mod->edit('user_id=' . $user_id, $edit_myjifen);
                        //添加日志
                        $log_text = '无手续费充值 in A Ex3'; //$this->visitor->get('user_name') . Lang::get('tongguoalipaychongzhi') . $total_fee . Lang::get('yuan');
                ...
                if ($done) {
                    $finalresult['result'] = 'success';
                    $finalresult['info'] = "found-in-a Ex3!";
                    exit(json_encode($finalresult));
                } else if (!$done && $count > 500) {
                    $req1->setPageNo("2");
                    $resp1 = $c->execute($req1, $accesstoken);
                    $datas = (array) $resp1;
                    foreach ($datas as $key => $val) {
...
//                            echo '站内充值成功';
                    ...
                    if ($done) {
                        ...
                        $finalresult['result'] = 'error';
                        $finalresult['info'] = "no-t; no-rs-f-in-a Ex3";
                        exit(json_encode($finalresult));
                    }
                } else {
                    $finalresult['result'] = 'error';
                ...
                //没有记录
                $finalresult['result'] = 'error';
                $finalresult['info'] = "no-t; no-rs-in-a Ex";
                exit(json_encode($finalresult));
            }
            $finalresult['result'] = 'success';
            $finalresult['total'] = $totalfee;
...
//            echo 'equal ';
    ...
    /**
     * 基于account的新版本
     * 查询今天或者昨天的 tom=yes查询昨天的
     * @return type 
     */
    public function checkMoney3() {
        //(1)得到两边今日的金额
        //支付宝的
        $paylog_mod = & m('paylog');
        $my_moneylog_mod = & m('my_moneylog');
        ...
        $tom = false;
        $url = 'http://' . MONEYSITE . '/index.php?app=default&act=financeSpec';
        if ($_GET['tom'] && $_GET['tom'] == 'yes') {
            $url = 'http://' . MONEYSITE . '/index.php?app=default&act=financeSpec1';
            $tom = true;
        }
        echo $url;
...
//                }
        ...
        $timestr = ' add_time > ' . gmstr2time(date('Y-m-d'));
        if ($tom) {
            $which = gmstr2time(date("Y-m-d", strtotime("-1 day")));
            $timestr = ' add_time < ' . gmstr2time(date('Y-m-d'));
            $timestr .= ' and  add_time > ' . $which;
        }
        $index = $my_moneylog_mod->find(array(
            'conditions' => 'leixing in(30, 40 ) and caozuo in (50,61,4)  and user_log_del = 0 and ' . $timestr,
            'order' => "id desc",
            'count' => true));
        $page['item_count'] = $my_moneylog_mod->getCount();
        foreach ($index as $var) {
        ...
        $checkresult['failed'] = 0;
        $oem = OEM;
//        $alimoney = $alimoney + 20;
        if ($total < $alimoney || !empty($_GET['do'])) {
//            echo 'in<br>';
            $index = $paylog_mod->find(array(
                'conditions' => 'createtime > CURDATE()-1 and trade_status =1 ',
                'limit' => $page['limit'],
...
//            var_dump($sitekey);
            //找出在ali而不在site的数字
            foreach ($aliM['spec'] as $stat => $v) {
//                echo '<br>one check: '.$stat;
                $t = in_array($stat, $sitekey, true);
//                if($t){
//                    echo 'the stat is iin'.$stat;
//                }else {
//                    echo 'not found n'.$stat;
//                }
                if (!$t) {
                    echo '<br> check the:' . $stat;
                    $checkresult['check'] = 'yes';
                    $time = $v['create_time'];
                    $money = $v['money'];
                    $key = $stat; //　ali_pay_order
                    /**
                     * time_to_sec(timediff('2015-06-16 13:19:22','2015-06-16 13:18:00'))  =82 
                     * select * from (select   time_to_sec(timediff(l.createtime,'2015-06-16 13:18:00')) as offset ,l.trade_no, l.out_trade_no,l.createtime  from ecm_paylog l where l.total_fee=5)a where a.offset > 0 and a.offset < 180;
                     */
                    $sqla = 'select * from (select   time_to_sec(timediff("' . $time . '",l.createtime)) as offset,l.total_fee ,l.trade_no, l.out_trade_no,l.createtime,l.customer_id,l.customer_name  from ecm_paylog l where l.total_fee=' . $money . ')a where a.offset > 0 and a.offset < 180';
//                    echo '<br>'.$sqla;
                    $resulta = $paylog_mod->db->getRow($sqla);
//                    echo '<br> res<br>';
//                    var_dump($resulta);
                    ...
                    if (!$resulta) {
                        ...
                        if ($ts['result'] == 'success' && !empty($ts['title'])) {
                            ...
                            $total_fee = $ts['totalfee'];
                            $time = $ts['create_time'];
                            $sqlts = 'select * from ecm_paylog where trade_no="' . $title . '"';
                            Log::write('in ts $sqlts : ' . $sqlts);
                            $resultts = $paylog_mod->db->getRow($sqlts);
                            if ($resultts) {
                                $user_id = $resultts['customer_id'];
...
//                                        'out_trade_no' => $stat,
                    ...
                    $url = "http://" . MONEYSITE . "/index.php?app=default&act=searchtrade&title=" . $title . "&totalfee=" . $total_fee . "&user_id=" . $user_id . "&user_name=" . $user_name . "&oem=" . $oem;
                    $url.= "&time=" . strtotime($resulta['createtime']);
                    $url = str_replace(' ', '%', $url); //时间带空格了
//                    echo '<br> the url is '.$url.'<br>';
                    $res = @json_decode($this->Get($url), true);
                    if ($res['result'] == 'success') {
                        $updateOuttrade = array(
                            'status' => 1, //成功
...
//       $result = @json_decode(Get($url),true);
    ...
    /**
     * 精确查询唯一的值：根据title,totalfee 与user_id确定；
     * 用在searchtrade 当在trade里查不到时，要在这里查，取代finance2
     * @param type $paylogmodel
     * @param type $user_id
     * @param type $user_name
     * @param type $totalfee
     ...
     * @param type $time 
     */
    function financeSpec4($paylogmodel, $user_id, $user_name, $totalfee, $title, $accesstoken, $oem, $time) {
        // 交易记录查询接口
        $c = new AopClient ();
        $c->appId = APP_ID;
        $c->rsaPrivateKeyFilePath = ROOT_PATH . "/rsa_private_key.pem";
        ...
        // 结束时间。与开始时间间隔在七天之内
        $end_time = date('Y-m-d H:i:s', $nowtime);
        $start_time = date('Y-m-d H:i:s', $nowtime - 1800);
        if ($time && !empty($time)) {
            $end_time = date('Y-m-d H:i:s', $time + 1800);
            $start_time = date('Y-m-d H:i:s', $time - 1);
        }
        $req1->setEndTime($end_time);
        // 支付宝订单号，为空查询所有记录
        // 开始时间，时间必须是今天范围之内
...
//        print_r($datas);
        ...
        if ($datas ['response_error']) {
            ...
            if ($count > 0) {
                for ($i = 0; $i < $count; $i++) {
                    $tradeinfo = $datas ['alipay_user_account_search_response'] ['account_records'] ['account_record'] [$i];
                    if ($tradeinfo ['business_type'] == 'other' && $tradeinfo ['type'] == 'payment' && $tradeinfo ['in_amount'] == $totalfee) {
                        $memo = $tradeinfo ['memo'];
                        if ($memo) {
                            $r = $this->parseMemo($memo, $titleG, $user_idG);
                            if (!$r || $user_id != $user_idG || $title != $titleG) {
                                continue;
                            }
                        } else {
                            continue;
...
//                            echo '站内充值成功';
                        ...
                        $my_money_mod->edit('user_id=' . $user_id, $edit_myjifen);
                        //添加日志
                        $log_text = '无手续费充值 in A 4 '; //$this->visitor->get('user_name') . Lang::get('tongguoalipaychongzhi') . $total_fee . Lang::get('yuan');
                ...
                if ($done) {
                    $finalresult['result'] = 'success';
                    $finalresult['info'] = "found-in-a Ex 4!";
                    exit(json_encode($finalresult));
                } else if (!$done && $count > 500) {
                    $req1->setPageNo("2");
                    $resp1 = $c->execute($req1, $accesstoken);
...
//                            echo '站内充值成功';
                    ...
                    if ($done) {
                        ...
                        $finalresult['result'] = 'error';
                        $finalresult['info'] = "no-t; no-rs-f-in-a Ex 4";
                        exit(json_encode($finalresult));
                    }
                } else {
                    $finalresult['result'] = 'error';
                ...
                //没有记录
                $finalresult['result'] = 'error';
                $finalresult['info'] = "no-t; no-rs-in-a Ex 4";
                exit(json_encode($finalresult));
            }
            $finalresult['result'] = 'success';
            $finalresult['total'] = $totalfee;
