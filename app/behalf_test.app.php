...
class Behalf_testApp extends MallbaseApp {
    /* 搜索商品 */
...
//        $t00 = time();
        ...
        if(empty($data)){
           ...
           /*  echo "good_list start";
            ...
            //$storenums = !isset($storenums)||empty($storenums)?15:$storenums;
            $storenums = 15;
            $goods_mod = & m('goods');
            ...
            while ($num < $storenums ) {
                ...
                $goods_list = $goods_mod->get_list2(array(
                    'conditions' => $conditions,
                    'order' => 'g.add_time DESC', // tyioocom
                    'fields' => 's.praise_rate,s.im_qq,s.im_ww,', // tyioocom
                    'limit' => $page['limit'],
                    'conditions_tt' => $param['keyword'],
                ), null, false, true, $total_found,$backkey);
          ...
          $cache_server->set($indexkey, $result, 7200);
        ...
        //dump($result);
        file_put_contents(ROOT_PATH.'/data/index_daily_new.goods', ecm_json_encode($result));
       return $result;
       //dump($goods_list);
    ...
    private function _get_behalf_area_stores()
    {
        ...
        $results = $model_storebehalfarea->getCol("SELECT store_id FROM ".$model_storebehalfarea->table." WHERE state='1'");
    ...
    function apply()
    {
        ...
        $storebehalfarea_model = & m('storebehalfarea');
    ...
    function store() {
        ...
        if ($isfirst || count($_GET) > 2) {
            ...
            $model_storebehalfarea = & m('storebehalfarea');
            $ba_stores_ids = $model_storebehalfarea->getCol("SELECT store_id FROM ".$model_storebehalfarea->table);
    ...
    //print_r($storelist);
        ...
        $this->display('behalf.goods.store.html');
    }
...
//        }
    ...
    function _get_group_by_info($param, $cached) {
        ...
        if ($data === false) {
            ...
            $data['by_category'] = array_slice($gcategories, 0, 7);
    ...
    function get_latestGoods_fromStore($storenums) {  
        ...
        $cache_server = & cache_server();
        $indexkey = 'behalf_goods_lastestGoods_from_store';
        $data = $cache_server->get($indexkey);
       
        if (!$data || empty($data)) {  
        
            $storenums = empty($storenums)?15:$storenums;
            $goods_mod = & m('goods');
    ...
    function test_list2()
    {
        ...
        $goods_mod = & m('goods');
        $goods_list = $goods_mod->get_list2(array(
            'conditions' =>'if_show=1 AND closed=0', //"g.if_show = 1 AND g.closed = 0 AND g.default_spec > 0 AND s.state = 1",
            //'order' => 'g.add_time desc',
            'fields' => 's.praise_rate,s.im_qq,s.im_ww,',
            'limit'=>"0,40",
            //'conditions_tt' =>''
