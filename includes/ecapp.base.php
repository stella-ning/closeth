...
define('BEHALF_GOODS_READY_APP',9); //代发商品已派单
define('BEHALF_GOODS_READY',10);//代发商品准备好了
define('BEHALF_GOODS_SEND',11);//代发商品已发货
define('BEHALF_GOODS_ADJUST',12);//代发商品换款中
...
define('BEHALF_GOODS_BACKING',51);//退货了
...
/*用户vip等级及下单数*/
//define('VIPONE_ORDERS',100);//用户下单达到100单为vip1
//define('VIPTWO_ORDERS',200);//用户下单达到200单为vip2
/* 订单状态 */
...
EOT;
    ...
    function sendMail($to,$subject,$message){
        ...
        $data['body'] = $message;
        $data['token'] = $member_token['token'];
        $data['url'] = 'http://121.199.182.35:30005/api/queue/mail/send';
...
//        var_dump($output);
    ...
    function sendSms($mobile ,$content){
        ...
        $data['token'] = $member_token['token'];
        $data['url'] = 'http://121.199.182.35:30005/api/queue/sms/send';
        $result = $this->curl_post($data ,1);
        return $result;
    }
...
log::write("noreply user_id:{$user_id}" , log::INFO);
    ...
    protected function curl_post( $params ,$post = 1 ){
        //    $url = 'http://nahuo.daifayuan.com/api/queue/order/invoice';
...
class BaseVisitor extends Object {
    ...
    function assign($user_info) {
        // zjh 此处改写behalf，为角色权限做处理
        $temp = $user_info;

        $behalf_1 = $this->_get_behalf($temp['user_id']);

        if(!$behalf_1){  // 判断当前账号是否为代发

            // 不是代发
            // 查找员工账号
            $employee = $this->_find_role_employee($temp['user_id']);

            if($employee){ // 存在员工

                $behalf_2 = $this->_get_behalf($employee['bh_id']);   

                if($behalf_2){  // 存在代发，并且代发是开启的

                    // 替换信息（关联到角色）
                    $temp['has_behalf'] = $behalf_2['bh_id'];
                    $temp['pass_behalf'] = $behalf_2['bh_allowed'];
                    $_SESSION[$this->_info_key] = $temp;
    ...
    // zjh 查找员工列表，用于判断该用户是否为员工，从而获取他对应的代发id
    function _find_role_employee($employee_id)
    {

        $_employee_role_mod = &m('employee_role'); 

        $employee = $_employee_role_mod->get(array(
    ...
    // zjh 获取代发信息
    function _get_behalf($bh_id)
    {
        $_behalf_mod = & m("behalf");

        $bh_info = $_behalf_mod->get(array(
