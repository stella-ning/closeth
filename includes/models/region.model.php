...
class RegionModel extends BaseModel
{
     ...
     * @return multitype:
     */
    function get_layer($region_id)
    {
    	$res = array();
    	while($region_id > 0)
    	{
    		$sql = "SELECT region_id,region_name,parent_id FROM ".$this->table." WHERE region_id=".$region_id;
    		$row = $this->getRow($sql);
    		if($row)
    		{
    			$res[] = array('region_id'=>$row['region_id'],'region_name'=>$row['region_name']);
     ...
     */
    function isleaf($region_id)
    {
    	$sql = "SELECT count(*) from `{$this->table}` WHERE parent_id = {$region_id}";
	...
	{
		if($region_id==1 || !$region_id){ // 第一级是中国的情况
		//if(!$region_id){
			return '全国';
		}
		$str = '';
