...
class GcategoryModel extends BaseModel
{
     ...
     * @date 2016-10-09
     */
    function get_list_limit($parent_id = -1, $shown = false,$store_id = false,$limit = 15)
    {
        $conditions = "1 = 1";
        $parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id'";
        $shown && $conditions .= " AND if_show = 1";
        $store_id && $conditions .= " AND store_id = 0";
...
class GcategoryBModel extends GcategoryModel
{
    ...
    function get_children($id, $cached = false)
    {
        ...
        {
            $sql = "SELECT cate_id, cate_name,cate_mname FROM {$this->table} WHERE parent_id = '$id' AND store_id = '{$this->_store_id}'";
            $res = $this->getAll($sql);
        }
    ...
    function get_ancestor($id, $cached = false)
    {
        ...
        if ($cached)
        {
            ...
            if ($id > 0 && isset($data[$id]))
            {
                while ($id > 0)
                {
                    $res[] = array('cate_id' => $id, 'cate_name' => $data[$id]['mname']?$data[$id]['mname']:$data[$id]['name']);
                    $id    = $data[$id]['pid'];
                }
            }
        }
        else
        {
            while ($id > 0)
            {
                $sql = "SELECT cate_id, cate_name,cate_mname, parent_id FROM {$this->table} WHERE cate_id = '$id' AND store_id = '{$this->_store_id}'";
                $row = $this->getRow($sql);
                if ($row)
                {
                    $res[] = array('cate_id' => $row['cate_id'], 'cate_name' =>$row['cate_mname']?$row['cate_mname']:$row['cate_name']);
                    $id    = $row['parent_id'];
                }
            }
        }
