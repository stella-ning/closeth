...
function keep(&$content , $rw){// true is read; or is write
 if($rw){
  $content = file_get_contents(ROOT_PATH.'/data/sphinx.dat');
  }
  else{
  file_put_contents(ROOT_PATH.'/data/sphinx.dat',$content);
  }
}
class Log {
    ...
    static function record($message,$level=self::ERR,$record=false) {
        if($record || strpos(LOG_LEVEL,$level)) {
