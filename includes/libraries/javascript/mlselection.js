    ...
    if (this.value > 0)
    {
        ...
        $.getJSON(url, {'pid':this.value}, function(data){
            if (data.done)
            {
                ...
                {
                    $("<select class='required' data-valid='isNonEmpty' date-tip = '' data-error='请选择地区'><option value=''>" + lang.select_pls + "</option></select>").change(regionChange).insertAfter(_self);
...
function marketInit(divId)
{
	$("#" + divId + " > select").change(marketChange); // select的onchange事件
	$("#" + divId + " > input:button[class='edit_market']").click(marketEdit); // 编辑按钮的onclick事件
}
    ...
    if (this.value > 0)
    {
        ...
        $.getJSON(url, {'pid':this.value}, function(data){
            if (data.done)
            {
                if (data.retval.length > 0)
                {
                    $("<select><option>" + lang.select_pls + "</option></select>").change(marketChange).insertAfter(_self);
                    var data  = data.retval;
                    for (i = 0; i < data.length; i++)
                    {
                        $(_self).next("select").append("<option value='" + data[i].mk_id + "'>" + data[i].mk_name + "</option>");
                    }
                }
            }
            else
