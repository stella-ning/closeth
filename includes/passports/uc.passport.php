...
class UcPassportPM extends BasePassportPM
{
    ...
    function send($sender, $recipient, $subject, $message, $replyto = 0)
    {
        ...
        $res = outer_call('uc_pm_send',array($sender, $recipient, '',$message, 1, 0, 0));
        if ($res == PRIVATEPMTHREADLIMIT_ERROR) {
            $this->_errors[] = array('msg' => get_msg('privatepmthreadlimit_error', array('limit' => '25'))); // 25 is defined at uc_client/data/cache/settings.php:privatepmthreadlimit
        }
        return $res;
    }
    /**
    ...
    function get_list($user_id, $page, $folder = 'newpm')
    {
        ...
        foreach ($rs['data'] as $value)
        {
            ...
            $result[$value['pmid']]['content'] = $value['lastsummary']; // wengxj: use subject as content seems wrong, not sure, try to use lastsummary instead
        }
