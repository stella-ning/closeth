...
class BaseOrder extends Object
{
     ...
     *
     */
    function _get_behalf_goods_list($order_id){
        if (!$order_id)
        {
            return array();
        }
        $orderbehalfgoods_model =& m('goodswarehouse');

        return $orderbehalfgoods_model->find("order_id={$order_id} AND goods_status not ".db_create_in(array(BEHALF_GOODS_ADJUST,BEHALF_GOODS_CANCEL)));
    }
    ...
    function _get_order_third($order_id){
        if (!$order_id)
        {
            return array();
        }

        $orderthird_model =& m('orderthird');

        return $orderthird_model->getOne('select third_id from '.$orderthird_model->table.' where order_id='.$order_id);
    }
    ...
    function _handle_consignee_info($goods_info, $post)
    {
        ...
        elseif(isset($consignee_info['shipping_choice']) && (intval($consignee_info['shipping_choice']) == 2))
        {
            ...
            //zjh 
            // $c = current($goods_info['items']);
            // $user_id = $c['user_id'];

            $my_deliverys = $behalf_mod->calculate_delivery_fee_bybehalf($post['behalf'],$post['gids'],$post['region_id']);
            foreach ($my_deliverys as $key => $value) {
                if($value['dl_id'] == $post['delivery']){
                    $shipping_fee = $value['dl_fee'];
                }
            }
