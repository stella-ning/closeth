...
//include(ROOT_PATH . '/eccore/log.php');
ecm_define(ROOT_PATH . '/data/log.setting.php');
import("log.lib");
...
function &sm($model_name, $params = array(), $is_new = false) {
    static $models = array();
    $model_hash = md5($model_name . var_export($params, true));
    if ($is_new || !isset($models[$model_hash])) {
        $model_file = ROOT_PATH . '/includes/models/' . $model_name . '.model.php';
...
//        echo '<br> '.$model_name;
        if ($is_new) {
            return new $model_name($params, sdb());
        }
        $models[$model_hash] = new $model_name($params, sdb());
    }

    return $models[$model_hash];
}
function &ntm($model_name, $params = array(), $is_new = false) {
    static $models = array();
    $model_hash = md5($model_name . var_export($params, true));
    if ($is_new || !isset($models[$model_hash])) {
        $model_file = ROOT_PATH . '/includes/models/' . $model_name . '.model.php';
...
//        echo '<br> '.$model_name;
        if ($is_new) {
            return new $model_name($params, ntdb());
        }
        $models[$model_hash] = new $model_name($params, ntdb());
    }

    return $models[$model_hash];
}
function &changm($model_name, $params = array(), $is_new = false) {
    static $models = array();
    $model_hash = md5($model_name . var_export($params, true));
    if ($is_new || !isset($models[$model_hash])) {
        $model_file = ROOT_PATH . '/includes/models/' . $model_name . '.model.php';
...
//        echo '<br> '.$model_name;
        if ($is_new) {
            return new $model_name($params, changdb());
        }
        $models[$model_hash] = new $model_name($params, changdb());
    }

    return $models[$model_hash];
}
function &sxm($model_name, $params = array(), $is_new = false) {
    static $models = array();
    $model_hash = md5($model_name . var_export($params, true));
    if ($is_new || !isset($models[$model_hash])) {
        $model_file = ROOT_PATH . '/includes/models/' . $model_name . '.model.php';
...
//        echo '<br> '.$model_name;
        if ($is_new) {
            return new $model_name($params, sxdb());
        }
        $models[$model_hash] = new $model_name($params, sxdb());
    }

    return $models[$model_hash];
}
function &mallm($model_name, $params = array(), $is_new = false) {
    static $models = array();
    $model_hash = md5($model_name . var_export($params, true));
    if ($is_new || !isset($models[$model_hash])) {
        $model_file = ROOT_PATH . '/includes/models/' . $model_name . '.model.php';
...
//        echo '<br> '.$model_name;
        if ($is_new) {
            return new $model_name($params, malldb());
        }
        $models[$model_hash] = new $model_name($params, malldb());
    }

    return $models[$model_hash];
}
function &dgm($model_name, $params = array(), $is_new = false) {
    static $models = array();
    $model_hash = md5($model_name . var_export($params, true));
    if ($is_new || !isset($models[$model_hash])) {
        $model_file = ROOT_PATH . '/includes/models/' . $model_name . '.model.php';
...
//        echo '<br> '.$model_name;
        if ($is_new) {
            return new $model_name($params, dgdb());
        }
        $models[$model_hash] = new $model_name($params, dgdb());
    }
...
function &changdb() {
    ...
    if ($db === null)
    {
        $cfg = parse_url(CHANGSHU_DB_CONFIG);
...
function getLocation() {
    ...
    $header = array(
        'apikey:6cd010ee9a34aaf4a12cf17252cede4c',
    );
    // 添加apikey到header
    ...
    return var_export(json_decode($res,true),true);
}
/**
 * 验证输入的邮件地址是否合法
 *
