 ...
 * @return  array
 */
class Zwd_article4_behalfWidget extends BaseWidget
{
    var $_name = 'zwd_article4_behalf';
    var $_ttl  = 86400;
        ...
        if($data === false)
        {
            ...
            $article_mod =& m('article');
				...
				$articles1 = $article_mod->find(array(
				   ...
				   'fields'=>'article_id, title,add_red',
				   'limit'=> $amount,
				...
				$orderbehalfs = $order_mod->findAll(array(
					'conditions'=>'bh_id > 0',
					'order'=>'order_id desc',
					'limit'=>'0,12'));
				foreach($orderbehalfs as $key=>$val)
				{
					...
					$val['buyer_name'] = ltrim($val['buyer_name'],'51t_');				
					$orderbehalfs[$key]['buyer_name'] =  mb_substr($val['buyer_name'],0,1)."**".mb_substr($val['buyer_name'],-1,1);
					if($val['seller_id'] == 0)
					{
						$tmp = str_replace(array(Lang::get('comma'), Lang::get('whitespace'), ' '), ',', $val['seller_name']);
						$keyword = explode(',', $tmp);
						$keyword = array_filter($keyword);
						sort($keyword);			
						$orderbehalfs[$key]['seller_name'] = $keyword[0];
					}
				}
